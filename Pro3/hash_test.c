...
{ 
	...
	int i; 
	long thread;
	for(thread_count=1;thread_count<21;thread_count++){
		time1=0.0;
		time2=0.0;
		time3=0.0;
		time4=0.0;
		...
		{ 
			...
			for(i=0;i<MAXN;i++){
				...
				for(thread=0;thread<thread_count;thread++){
					switch((argv[1][0]-'0')){
						case 1:
							pthread_create(&thread_handles[thread],NULL,t_hash1,(void*)thread);
							break;
						case 2:
							pthread_create(&thread_handles[thread],NULL,t_hash2,(void*)thread);
							break;
						case 3:
							pthread_create(&thread_handles[thread],NULL,t_hash3,(void*)thread);
							break;
					}
